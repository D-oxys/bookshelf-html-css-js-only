...
function refreshBookshelves() {
  ...
  const unreadBooks = searchResults.filter((book) => !book.isComplete);
  const readBooks = searchResults.filter((book) => book.isComplete);
...
searchInput.addEventListener("keypress", (event) => {
  if (event.key === "Enter") {
    ...
    // Panggil fungsi searchBooks di sini
